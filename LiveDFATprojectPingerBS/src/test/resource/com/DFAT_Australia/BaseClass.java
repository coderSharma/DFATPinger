...
public abstract class BaseClass {
	...
	public WebDriver browserStackRandomSetup() throws MalformedURLException, InterruptedException {
		
		Random Browser = new Random ();
		int BrowserType=Browser.nextInt(3);
		...
		caps.setCapability("resolution", "1024x768");
		caps.setCapability("browserstack.debug", "true");
		caps.setCapability("project", "DFAT Pinger");
		caps.setCapability("build", "TeamCity Maven Build");
		driver = new RemoteWebDriver(new URL("http://vjqa:z5fjTxhgdi3xda1YPAm2@hub.browserstack.com/wd/hub"),caps);
		driver.manage().window().maximize();
		driver.manage().timeouts().implicitlyWait(15, TimeUnit.SECONDS);
		driver.get(fullurl);
